...
const Landing = () => {
  ...
  useEffect(() => {
    ...
    .then(response => {
      ...
      const arr = data.popular_game;
      // console.log(arr);
      const newImages = arr.map((popular_game) => popular_game[1]);
      setPopularGame(newImages);
    })
  ...
  const handleSubmit = (event) => {
    ...
    .then(data => {
      ...
      // console.log(arr);
      const newImages = arr.map((result) => result[1]);
      setImages(newImages);
    })
    .catch(error => {
